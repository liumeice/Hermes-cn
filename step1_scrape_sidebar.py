    ...
    return page.evaluate('''() => {
        ...
        function walk(li, parentGroup) {
            ...
            if (linkEl) {
                ...
                if (text && text.length >= 2 && href && href.indexOf('/docs/zh-Hans/') >= 0 && !seen.has(href)) {
                    seen.add(href);
                    const levelMatch = li.className.match(/level-(\d)/);
                    return {
                        title: text,
...
def main():
    ...
    output_json = {
        'source': BASE_URL,
        'root': 'Hermes Agent Documentation (zh-Hans)',
        'total_pages': total,
    ...
    # Output sidebar.md
    md_lines = [
        '# Hermes Agent Documentation - Table of Contents (zh-Hans)',
        '',
        f'Source: {BASE_URL}',
        f'Total pages: {total}',
        '',
        f'- [Hermes Agent 文档]({BASE_URL})',
        '',
    ]
