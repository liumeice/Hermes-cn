#!/usr/bin/env python3
"""
Step 2 (Multi-threaded): Generate individual PDFs for each documentation page + a cover PDF.
...
Uses ThreadPoolExecutor to process pages in parallel, one Playwright context per thread.

Usage:
  source venv/Scripts/activate
  python step2_generate_pdfs_mt.py              # Auto-detect CPU count
  python step2_generate_pdfs_mt.py --threads 4  # Use 4 threads
"""
...
# ============================================================
# DOM manipulation script (same as single-threaded version)
# ============================================================
DOM_MANIPULATE_JS = """
function() {
  ...
  document.querySelectorAll('*').forEach(function(el) {
    var text = el.textContent || '';
    if ((text.includes('此页面对您有帮助吗') || text.includes('Was this page helpful')
         || text.includes('此页面是否有帮助'))
  ...
  document.querySelectorAll('*').forEach(function(el) {
    var text = el.textContent || '';
    if ((text.trim() === '本页内容' || text.trim() === 'On this page' || text.trim() === '目录')
  ...
  bgStyle.textContent = [
    ...
    'h1, h2, h3, h4, h5, h6 { font-family: "SimHei", "黑体", sans-serif !important; }',
...
<div class="page">
  ...
  <div class="center-wrap"><div class="content">
    ...
    <div class="features"><span class="feature-tag">快速开始</span><span class="feature-tag">核心概念</span><span class="feature-tag">工具使用</span><span class="feature-tag">多步推理</span><span class="feature-tag">MCP 协议</span><span class="feature-tag">API 参考</span></div>
  </div></div>
...
def generate_page_pdf_worker(idx, total, title, url, output_path, lock):
    """
    Worker function: launch own Playwright instance to generate one PDF.
    Each thread gets its own browser + context for thread safety.
    Returns (idx, title, status, size_kb).
    status is one of 'success', 'skipped', 'failed'.
    """
    if output_path.exists() and output_path.stat().st_size > 0:
        with lock:
            print(f'    [{idx:3d}/{total}] ⏭ Skip: {title}')
        return (idx, title, 'skipped', 0)
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                if is_404:
                    ...
                    browser.close()
                    with lock:
                        print(f'    [{idx:3d}/{total}] 404: {title}')
                    return (idx, title, 'failed', 0)
                ...
                return (idx, title, 'success', size_kb)
            except Exception as e:
                context.close()
                browser.close()
                with lock:
                    print(f'    [{idx:3d}/{total}] {title:<50s} FAILED: {e}')
                return (idx, title, 'failed', 0)
    except Exception as e:
        with lock:
            print(f'    [{idx:3d}/{total}] {title:<50s} FAILED (browser): {e}')
        return (idx, title, 'failed', 0)


def main():
    parser = argparse.ArgumentParser(description='Step 2: Generate individual PDFs (multi-threaded)')
    parser.add_argument(
        '--threads', '-t',
        type=int,
        default=None,
    ...
    max_workers = args.threads if args.threads is not None else cpu_count
    print(f'  CPU cores detected: {cpu_count}')
    print(f'  Worker threads: {max_workers}')
    print()

    print('Step 2 (MT): Generating individual PDFs')
    print()
    ...
    # Generate cover PDF (single-threaded, same as before)
    ...
    # Generate page PDFs using thread pool
    print(f'  Generating page PDFs with {max_workers} threads...')
    print()
    ...
    for idx, page_data in enumerate(pages, 1):
        ...
        tasks.append((idx, total, title, url, output_path))

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(generate_page_pdf_worker, idx, total, title, url, output_path, print_lock): (idx, title)
            for idx, total, title, url, output_path in tasks
        }
